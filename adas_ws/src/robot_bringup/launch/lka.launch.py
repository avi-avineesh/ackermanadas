#!/usr/bin/env python3
"""
lka.launch.py — LKA Test Launch
═════════════════════════════════
t=0s   Gazebo Harmonic (lka_world)
t=8s   spawn_ego  at x=0.3 y=0.0 z=0.15 (facing East)
...
def generate_launch_description():
    ...
    world_file = os.path.join(pkg_share, 'worlds', 'lka_world.sdf')
    ...
    spawn = TimerAction(
        ...
        actions=[ExecuteProcess(
            cmd=[
                'ros2', 'run', 'robot_bringup', 'spawn_ego',
                '--ros-args',
                '-p', 'world:=lka_world',
                '-p', 'x:=0.3',
                '-p', 'y:=0.0',
